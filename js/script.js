...
function initDateTabs() {
    ...
    // ОПТИМИЗАЦИЯ: собираем HTML в строку, вставляем один раз
    let htmlContent = '';
    const now = new Date();
...
function renderRides() {
    ...
    // Собираем весь HTML в одну строку, чтобы браузер не тормозил
    let ridesHtml = '';
...
function renderSeatMap(ride) {
    ...
    // Используем Fragment для быстрой отрисовки без перерисовки браузера
    ...
    finalLayout.forEach(seatNum => {
        ...
        // УБРАН ХОВЕР ОТ TAILWIND И УВЕЛИЧЕНИЕ. ДОБАВЛЕН НАШ КЛАСС seat-hover-effect
    ...
    // Вставляем все места разом (очень быстро)
...
function toggleSeat(seatNum, btn) {
    ...
    if (index > -1) {
        ...
        // Убираем ховер с выделенного места, чтобы не мешался
