...
// 🌍 СЛОВАРЬ ПЕРЕВОДОВ (ТЕПЕРЬ ТУТ ЕСТЬ ВООБЩЕ ВСЁ)
const translations = {
    ru: {
...
function changeLanguage(lang) {
    ...
    // Если модалка открыта, принудительно перерисовываем схему мест, чтобы перевелась надпись "Дверь"
...
function renderSeatMap(ride) {
    ...
    finalLayout.forEach(seatNum => {
        ...
        if (seatNum === 'door_row') {
            ...
            // ТУТ ИСПОЛЬЗУЕМ СЛОВАРЬ И DATA-LANG ДЛЯ ДВЕРИ
            doorRow.innerHTML = `
        ...
        btn.className = `w-full aspect-square rounded-t-xl rounded-b-md font-bold text-sm md:text-base flex items-center justify-center transition-all shadow-sm border-b-4 `;
        
        if (isBooked) {
            btn.className += 'bg-gray-200 text-gray-400 border-gray-300 cursor-not-allowed';
        } else {
            btn.className += 'bg-white text-gray-700 border-gray-200 hover:border-blue-300 hover:bg-blue-50 cursor-pointer';
...
function toggleSeat(seatNum, btn) {
    ...
    if (index > -1) {
        ...
        btn.classList.remove('bg-[#1e3a8a]', 'text-white', 'border-blue-900', 'shadow-md', 'scale-105');
        btn.classList.add('bg-white', 'text-gray-700', 'border-gray-200');
    } else {
        if (currentlySelectedSeats.length >= 5) {
            alert(translations[currentLang].alert_limit); // Алерт из словаря
            return;
        }
        currentlySelectedSeats.push(seatNum);
        btn.classList.remove('bg-white', 'text-gray-700', 'border-gray-200');
        btn.classList.add('bg-[#1e3a8a]', 'text-white', 'border-blue-900', 'shadow-md', 'scale-105');
    }
...
async function confirmBooking() {
    ...
    confirmBtn.innerText = translations[currentLang].btn_processing; // "Оформляем..." из словаря
    ...
    if (error) {
        ...
        alert(translations[currentLang].alert_error); // Алерт ошибки
