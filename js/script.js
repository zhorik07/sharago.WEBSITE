...
function openModal(id) {
    ...
    // --- МАГИЯ ПЕРЕВОДА ОПИСАНИЯ ВОДИТЕЛЯ ---
    // По умолчанию ставим базовый текст из словаря
    let driverDesc = translations[currentLang].driver_default_desc;
    
    // Подменяем на текст из базы, если он там есть
        ...
        if (seatNum === 'door_row') {
            ...
            doorRow.innerHTML = `
                ...
                <div class="bg-blue-50 text-blue-700 text-[10px] font-bold uppercase tracking-wider pl-3 pr-2 py-1.5 rounded-l-xl border-y border-l border-blue-200 shadow-sm flex items-center gap-1 absolute -right-[18px] z-10">
                    Дверь <i class="fa-solid fa-arrow-right-from-bracket"></i>
                </div>
            `;
...
function toggleSeat(seatNum, btn) {
    ...
    if (index > -1) {
        ...
        if (currentlySelectedSeats.length >= 5) {
            alert("Больше 5 мест за раз забронировать нельзя, брат!");
            return;
        }
...
async function confirmBooking() {
    ...
    if (currentlySelectedSeats.length === 0) { alert("Выбери хотя бы одно место на схеме!"); return; }
    ...
    if (error) {
        ...
        confirmBtn.innerText = "Подтвердить бронь";
        return;
    }
